...
template <typename T> class Dynamic_Array
{
	public:
      ...
      *************************************************************************/
      ...
      template <typename U> friend std::ostream& operator << (std::ostream& os, const Dynamic_Array<U>& src);
      ...
      *************************************************************************/
		...
		Dynamic_Array<T>* deserialize(std::string serialized_da, T(*StringToT)(std::string)) const;
...
CONSTRUCTORS
*******************************************************************************/
template <typename T> Dynamic_Array<T>::Dynamic_Array()
{
   this->size          = 4;
...
template <typename T> Dynamic_Array<T>::Dynamic_Array(std::size_t initial_size)
{
...
template <typename T> Dynamic_Array<T>::Dynamic_Array(const Dynamic_Array<T>& src)
{
...
template <typename T> Dynamic_Array<T>::Dynamic_Array(Dynamic_Array<T>&& src) noexcept
{
   // Obtain all member variables from the src Dynamic Array
   this->size          = src.size;
   this->maximum_index = src.maximum_index;
...
SET FUNCTIONS FOR DATA STRUCTURE PROPERTIES
*******************************************************************************/
template <typename T> void Dynamic_Array<T>::set_resize_factor(std::size_t resize_factor)
{
...
GET FUNCTIONS FOR DATA STRUCTURE PROPERTIES
*******************************************************************************/
template <typename T> int Dynamic_Array<T>::get_resize_factor()
{
   // Return this dynamic array's resize factor.
   return this->resize_factor;
}

template <typename T> int Dynamic_Array<T>::get_size()
{
   // Returns the size of this dynamic array.
   return this->size;
}

template <typename T> int Dynamic_Array<T>::get_maximum_index()
{
...
DATA STRUCTURE OPERATIONS
*******************************************************************************/
template <typename T> void Dynamic_Array<T>::insert(const T& data, int index)
{
   ...
   {
      throw std::out_of_range("Tried to insert data from an index higher than one position after data was stored.");
   }
...
template <typename T> void Dynamic_Array<T>::replace(const T& data, int index)
{
   // Check if the replacement takes place within the existing index range.
   // If not throw an exception.
   if (index > this->maximum_index)
   {
      throw std::out_of_range("Tried to replace data from an index higher than data was stored.");
   }
   else
   {
...
template <typename T> void Dynamic_Array<T>::remove(int index)
{
   // Check if the removal takes place within the existing index range.
   // If not throw an exception.
   if (index > this->maximum_index)
   {
      throw std::out_of_range("Tried to remove data from an index higher than data was stored.");
   }
   else
   {
...
template <typename T> void Dynamic_Array<T>::merge(Dynamic_Array& src)
{
...
DATA STRUCTURE OPERATOR OVERLOADS
*******************************************************************************/
template <typename T> T& Dynamic_Array<T>::operator [] (int index) const
{
   if (index > this->maximum_index)
   {
      throw std::out_of_range("Tried to set data from an index higher than data was stored.");
   }
   else
   {
...
template <typename U> std::ostream& operator << (std::ostream& os, const Dynamic_Array<U>& src)
{
...
template <typename T> Dynamic_Array<T>& Dynamic_Array<T>::operator = (const Dynamic_Array& src) {
...
template <typename T> Dynamic_Array<T>& Dynamic_Array<T>::operator = (Dynamic_Array&& src) noexcept {
...
DATA STRUCTURE SEARCH ALGORITHMS
*******************************************************************************/
template <typename T> int Dynamic_Array<T>::linear_search(T& data, int offset)
{
   // Checks if the start of the search is in the valid range of data-filled
   // indices.
   if (offset > maximum_index)
   {
      throw std::out_of_range("Offset parameter was greater than the maximum_index.");
   }
...
DATA STRUCTURE SERIALIZATION AND DESERIALIZATION
*******************************************************************************/
template <typename T> std::string Dynamic_Array<T>::serialize(std::string(*TtoString)(T)) const
{
...
template <typename T> Dynamic_Array<T>* Dynamic_Array<T>::deserialize(std::string serialized_da, T(*StringToT)(std::string)) const
{
	...
	while(position != std::string::npos)
	{
		...
		if (search != std::string::npos)
		{
         ...
         // Dynamic Array. Then set the position to the end of the word.
			...
			deserialized_da->insert(StringToT(serialized_da.substr(position, substring_length)));
			position = search + 1;
		}
		else
		{
         ...
         // into the Dynamic Array.
			std::size_t substring_length = (sizeof(serialized_da) - 2) - (position + 1);
			deserialized_da->insert(StringToT(serialized_da.substr(position, substring_length)));
